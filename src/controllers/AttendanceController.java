...
public class AttendanceController {
    ...
    public AttendanceController() {
        ...
        char eFile = new InputHelper().readCharacter("Load data from the already existing Attendance file [y/n]","YN");
        if(eFile == 'Y' || eFile == 'y')
        {
...
Scanner input = new Scanner(System.in);
        ...
        switch(option){
            case "1":
                ...
                Swipe swipe = new Swipe(temp, room);
    ...
    private void listSwipeStatistics() 
    {
        ...
        String room = new InputHelper().readString("Enter room name");
        DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
        Calendar time = Calendar.getInstance();  
        for(int i  = this.repository.getItems().size()-1; i >-1; i--)
        {
            if(this.repository.getItems().get(i).getRoom().equals(room))
            {
                time =  this.repository.getItems().get(numberOfSwipes).getSwipeDateTime();
                numberOfSwipes++;
            }
        }
        
            System.out.println("The number of swipes for " + " the ROOM " + room + " is " + numberOfSwipes + "."+ "\n" + "Last time swiped: " + dateFormat.format(time.getTime()));
    }     
}
